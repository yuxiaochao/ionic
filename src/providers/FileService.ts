...
export class FileService {
  ...
  deleteById(id: string): Observable<FileObj> {
    ...
    return this.httpService.get('/deleteById', {id: id});
  }
  ...
  getFileInfoByIds(ids: string[]): Observable<FileObj[]> {
    ...
    return this.getFileCacheByIds(ids).mergeMap(cacheData => {
      ...
      return this.httpService.get( '/getByIds', {ids: queryIds}).map(result => {
        if (!result.success) {
          ...
          for (let fileObj of result.data) {
            ...
            fileObj.thumbPath =  fileObj.thumbPath;
          }
  ...
  uploadMultiByBase64(fileObjList: FileObj[]): Observable<FileObj[]> {
    ...
    return this.httpService.post('/appUpload?directory=ionic2_tabs', fileObjList).map(result => {
      if (!result.success) {
        this.nativeService.alert(result.msg);
        return [];
      } else {
