...
export class NativeService {
  ...
  isBackground: boolean = false;//是否在后台运行
  /**
   * 监听程序是在前台还是在后台运行
   */
   ...
   */
  isWindows(): boolean {
    return this.isMobile() && this.platform.is('windows');
  }
  ...
  /**
   * 创建多层目录
   * @param {string} systemPath  系统目录
   * @param {string} dirPath  自定义目录
   */
  createDirectoryMultiple(systemPath: string, dirPath: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      if (dirPath) {
        let currPath = "";
        let newPath = "";
        if (/\//g.test(dirPath)) {//包含斜杠/
          let paths = dirPath.split("/");
          currPath = paths.shift();//删除第一个数组元素
          newPath = paths.join("/");//新的路径
        } else {
          currPath = dirPath;
          newPath = "";
        }
        this.file.checkDir(systemPath, currPath).then(req => {
          this.createDirectoryMultiple(systemPath + currPath + "/", newPath).then(req03 => {
            resolve(req03);
          }).catch(err03 => {
            reject(err03);
          });
        }).catch(err => {
          this.file.createDir(systemPath, currPath, false)
            .then(req02 => {
              this.createDirectoryMultiple(systemPath + currPath + "/", newPath).then(req03 => {
                resolve(req03);
              }).catch(err03 => {
            ...
            reject("createDirectoryMultiple createDir err02:" + err02.message + ":" + systemPath);
          });
        });
      } else {
        resolve(true);
  ...
  noticeId: number = 1;
  noticeIdMap: any = {};//信息id集合
  /**
   * 信息栏通知
   */
  notice(title: string = '应用通知', text: string = '', data: any = {}, id: string = '1', type: string = 'chat') {
    let mapid: number = this.noticeIdMap[id + '_' + type];
    if (!mapid) {
      mapid = this.noticeId;
      this.noticeIdMap[id + '_' + type] = this.noticeId;
      this.noticeId++;
    }
    //通知栏通知
   ...
   * @param {number} time
   */
  vibrate(time: number = 500) {
    this.vibration.vibrate(time);//震动0.5秒
  }

  /**
   * 通知声
   * @param type 0,默认音效  1,系统消息提示音  2,特特别关心
   */
  preloadSimple(type?:number) {
    //uniqueId1为音频文件的唯一ID
    //assetPath音频资产的相对路径或绝对URL（包括http：//）
    //官网还有更多的配置，这里只需要两个参数就行了，后面的回调记得带上
    let nativeAudioBeep = this.globalData.nativeAudioBeep;
    if(type==1){//系统消息提示音
      nativeAudioBeep.play('systemBeep', () => {
        this.nativeAudio.stop('systemBeep').then(() => {});
      });
    }else if(type==2){//特特别关心
      nativeAudioBeep.play('veryConcernedBeep', () => {
        this.nativeAudio.stop('veryConcernedBeep').then(() => {});
      });
    }else {//默认音效
      nativeAudioBeep.play('defaultBeep', () => {
        this.nativeAudio.stop('defaultBeep').then(() => {});
      });
    }
  }
  ...
  testFileFun() {
    let msg = "";
    //安装应用程序的只读目录
    let applicationDirectory = this.file.applicationDirectory;
    //安装应用程序的只读目录
    let applicationStorageDirectory = this.file.applicationStorageDirectory;
    //在哪里放置应用程序特定的数据文件
    let dataDirectory = this.file.dataDirectory;
    //应用程序重新启动后应该保留的缓存文件。应用程序不应该依赖操作系统来删除此处的文件
    let cacheDirectory = this.file.cacheDirectory;
    //Android：外部存储的应用程序空间
    let externalApplicationStorageDirectory = this.file.externalApplicationStorageDirectory;
    //Android：将应用程序特定的数据文件放在外部存储上的位置
    let externalDataDirectory = this.file.externalDataDirectory;
    //Android：外部存储上的应用程序缓存
    let externalCacheDirectory = this.file.externalCacheDirectory;
    //iOS：操作系统可以随意清除的临时目录
    let tempDirectory = this.file.tempDirectory;
    //iOS：保存应同步的应用程序特定文件（例如，与iCloud）
    let syncedDataDirectory = this.file.syncedDataDirectory;
    //iOS：应用程序专用的文件，但对其他应用程序有意义（例如Office文件）
    let documentsDirectory = this.file.documentsDirectory;

    msg += "applicationDirectory:" + applicationDirectory + "    applicationStorageDirectory:" + applicationStorageDirectory + "     ";
    msg += "dataDirectory:" + dataDirectory + "    cacheDirectory:" + cacheDirectory + "     ";
    msg += "externalApplicationStorageDirectory:" + externalApplicationStorageDirectory + "    externalDataDirectory:" + externalDataDirectory + "     ";
    msg += "externalCacheDirectory:" + externalCacheDirectory + "    tempDirectory:" + tempDirectory + "     ";
    msg += "syncedDataDirectory:" + syncedDataDirectory + "     documentsDirectory:" + documentsDirectory + "      ";
    try {
      //以字节获得可用磁盘空间
      this.file.getFreeDiskSpace().then(req => {
        msg += "getFreeDiskSpace:" + req + "    ";
      });
      //检查目录中是否存在某个目录
      this.file.checkDir("data", "data").then(req => {
        msg += "      checkDir:" + req;
      })
      //在特定路径中创建一个新目录
      this.file.createDir("data", "data123", false).then(req => {
        msg += "      createDir:" + req;
      }).catch(err => {
        msg += "      createDir error:" + err;
      })
      this.file.listDir("data", "data").then(req => {
