...
@IonicPage()
@Component({
  selector: 'page-answer-card',
  templateUrl: 'answer-card.html',
})
export class AnswerCardPage {
...
//初始化缓存数据
  ...
  submitExam(){
    //验证至少答题X分钟是否可提交
    let tact013 = this.paper['TACT013'];
    if( tact013 != 0)
      if(this.testtime < tact013*60){
        this.nativeService.alert('','','开考'+tact013+'分钟后才能交卷');
        return ;
      }
