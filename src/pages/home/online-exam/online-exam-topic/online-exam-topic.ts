...
@Component({
  selector: 'online-exam-topic',
  templateUrl: 'online-exam-topic.html'
})
export class OnlineExamTopicComponent{
  ...
  //初始化不定项选择
  initUncert(){
    if(this.answer_object['uncert_'+this.topic.ID]){
      this.uncert = "#ffc125";
    }else{
      this.uncert = "";
    }
  ...
  selectedUncert(){
    this.storage.get(this.answer_key).then(data=>{//获取缓存数据
      this.answer_object = data || {};
      this.answer_object['uncert_'+this.topic.ID] = !this.answer_object['uncert_'+this.topic.ID];
      if(this.answer_object['uncert_'+this.topic.ID]){
        this.uncert = "#ffc125";
      }else{
        this.uncert = "";
