...
/**
 * Generated class for the ErrorTopicAnswerCardPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-error-topic-answer-card',
  templateUrl: 'error-topic-answer-card.html',
})
export class ErrorTopicAnswerCardPage {
    ...
    this.storage.get(this.answer_key).then(data=>{
      if(data){
        ...
        for(let i=0;i<this.topic.length;i++){
          answer = data['answer_result_'+this.topic[i]['ID']];
