...
@IonicPage()
@Component({
  selector: 'page-tasksign',
  templateUrl: 'tasksign.html',
})
export class TasksignPage {
  ...
  toSubmit() {
    if (this.zb == undefined || this.zb == ''||this.zb == 'undefined') {
      this.initLocation();
      return this.nativeService.alert("当前定位失败！请稍后重试");
    } else if (this.imageDataitem.length == 0) {
